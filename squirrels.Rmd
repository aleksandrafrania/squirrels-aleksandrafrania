...
```{r, echo = TRUE}
map2 <- get_stadiamap(borders, zoom = 14, maptype = "stamen_terrain_lines")
...
```{r, echo = FALSE}
map3 <- get_stadiamap(borders, zoom = 14, maptype = "stamen_toner_lines")

ggmap(map3) +
  geom_point(data = squirrels, 
             aes(x = X, 
                 y = Y, 
                 colour = Primary.Fur.Color), 
             size = 0.5) +
  ...
  labs(title = "Squirrel fur colors in Central Park", 
       x = NULL, 
       y = NULL) +
  theme_minimal()
